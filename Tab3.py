...
class TabThree:
    def __init__(self, notebook):
        ...
        ttk.Frame(leftside_tab, height=35).grid(column=0, row=2)
        ...
        ttk.Frame(leftside_tab, height=35).grid(column=0, row=4)

        edgesframe = ttk.Frame(leftside_tab)
        edgesframe.grid(column=0, row=5)
        ttk.Label(edgesframe, text="Average num of edges: ").grid(column=0, row=0)
        self.edges_var = IntVar()
        edges_var = self.edges_var
        edges = ttk.Entry(edgesframe, textvariable=edges_var, width=6)
        edges.grid(column=1, row=0)
        self.edges = edges

        ttk.Frame(leftside_tab, height=35).grid(column=0, row=6)

        runsframe = ttk.Frame(leftside_tab)
        runsframe.grid(column=0, row=7)
        ...
        ttk.Frame(leftside_tab, height=35).grid(column=0, row=8)

        buildframe = ttk.Frame(leftside_tab)
        buildframe.grid(column=0, row=9)
        ttk.Label(buildframe, text="Create connected graph? ").grid(column=0, row=0)
        self.build_var = IntVar()
        build_var = self.build_var
        check = ttk.Checkbutton(buildframe, text='',variable=build_var)
        check.grid(column=1, row=0)

        ttk.Frame(leftside_tab, height=35).grid(column=0, row=10)

        buttonframe = ttk.Frame(leftside_tab)
        buttonframe.grid(column=0, row=11)
        ...
        statusframe.grid(column=0, row=12)
    ...
    def stun_func(self):
        if self.run is False:
            if self.n_var.get() % 2 == 1 and self.edges_var.get() % 2 == 1:
                self.status_var.set("N and Average num of edges\ncan't be odd at the same time")
                return
    ...
    def task(self):
        ...
        for i in range(number_of_runs):
            ...
            self.graph = Board(self.n_var.get(), self.color_var.get())
            self.graph.reset_tiles()
            while self.run:
                if not self.graph.is_uniformed():
                    ...
                    break
            self.listbox_list.append(str(i) + "                                   " + str(self.graph.rounds))
            self.listbox_var.set(self.listbox_list)
    ...
    def task2(self):
        ...
        self.graph = Graph_Logic.Graph(self.n_var.get(), self.edges_var.get(), self.color_var.get(), self.build_var.get())
        if not self.graph.is_connected():
            self.listbox.see("end")
            self.listbox_list.append("     " + str(self.i) + "                                          " + "skip")
            self.listbox_var.set(self.listbox_list)
        ...
        if self.run:
            ...
            self.listbox.see("end")
            self.listbox_list.append("     " + str(self.i) + "                                          " + str(self.graph.rounds))
            self.listbox_var.set(self.listbox_list)
            self.rounds_list.append(self.graph.rounds)
            if self.i < self.runs_var.get():
                self.i += 1
                _thread.start_new_thread(self.task2, ())
